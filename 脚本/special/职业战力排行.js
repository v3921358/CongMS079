...
function action(mode, type, selection) {
	...
	if (status == 0) {
		var text = "#d┏━━━━━━━━职业战力一周排名━━━━━━━━┓#k\r\n\r\n";
		
        text += "#L1#本周排行#l\t\t";
        text += "#L2#上周排行#l\t\t";
        text += "#L3#上传战力#l\r\n\r\n";
		text += format(" ", 30, "") + format(" ", 20, "#r当前本人战力值：" + getEquipScore()) + "\r\n\r\n"; 
		text += "#d     每一星期刷新一次  排在第一名  领取专属奖励#k\r\n";
		text += "#d┗━━━━━━━━━━━━━━━━━━━━━━━━┛#k";
		//cm.gainGP(45);
        cm.sendSimple(text);
		...
		if (selection == 3) {
			cm.dispose();
			cm.openNpc(9040008, "职业战斗力信息上传")
		} else {
			var txt = "请选择查看的职业：\r\n";
			for (var i in jobs) {
				txt += "#L"+i+"##r["+jobs[i][0]+"]#l "
			}
			cm.sendOk(txt);
		}
	} else if (status == 2) {
        var dbName = "";
		if (sel == 1) {
			dbName = "equip_score_job"
		} else {
			dbName = "equip_score_job_before"
		}
		var selectSql = "select t.character_id,t.job,t.name,t.total_score from " + dbName + " t order by t.total_score desc ";
		...
		for (var i in list) {
			...
			if (jobs[selection][1].indexOf(parseInt(list[i].get("job")/100)) < 0) {
				continue;
			} else {
				j++;
			}
			...
			txt += " ";
			txt += "#r"+format(" ", 13, ("第 "+j+" 名").toString());
            ...
            if (!selfIn) {
                ...
                if (hour >= 21 && false) {
                    ...
                    if (paiming < 0) {
						...
						cm.addFromDrop(eq);
                        cm.setBossLog2("职业战力排行");
                        cm.getPlayer().dropMessage(1, "领取成功");
                    } else {
                        cm.sendOk("前"+giftid.length+"名才可以领取奖励");
                    }
...
function getEquipScore() {
	var sql = cm.sql_Select("select total_score from equip_score_job where character_id = ?", cm.getPlayer().getId());
	if (sql.length > 0) {
...
function getlog () {
	var sql = cm.sql_Select("SELECT * FROM bosslog2 WHERE characterid = ? AND bossid = '职业战力排行' AND WEEKOFYEAR(time) = WEEKOFYEAR(NOW())", cm.getPlayer().getId());
	return sql.length;
}
