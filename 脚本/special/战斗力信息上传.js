...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        var selectSql = "select t.* from equip_score t where character_id=" + characterId;
        var list = cm.sql_Select(selectSql);
        if (list.size() == 0) {
            var mainIsertSql = "insert into equip_score(character_id,total_score,job,name) values (?, ?, ?, ?)";
            ...
            if (hisScore != totalScore) {
                //update
                var mainUpdateSql = "update equip_score set total_score=" + totalScore + " ";
