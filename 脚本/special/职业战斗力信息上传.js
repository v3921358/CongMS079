...
var fourAttributeRule = [//四维 每1点获得1分
	[1, 1], //战士
	[1, 1], //法师
	[1, 1], //弓箭手
	[1, 1], //飞侠
...
var aggressivityRule = [//双攻 每1点获得2分
	[1, 2], //战士
	[1, 2], //法师
	[1, 2], //弓箭手
	[1, 2], //飞侠
...
function action(mode, type, selection) {
    ...
    if (status == 0) {
		...
		if (inx < 0) {
			cm.getPlayer().dropMessage(5, "你的职业不参与战斗力信息统计，请联系管理员，职业id["+job+"]");
			cm.dispose();
			return;
		}
        ...
        for (var i in checkList) {
            ...
            if (obj != null) {
                ...
                if (true)//四维
                {
                    ...
                    var fourAttributeScore = Math.floor(avg / fourAttributeRule[inx][0]) * fourAttributeRule[inx][1];
                ...
                if (true)//双攻
                {
                    ...
                    var aggressivityScore = Math.floor(avg / aggressivityRule[inx][0]) * aggressivityRule[inx][1];
        ...
        var selectSql = "select t.* from equip_score_job t where character_id=" + characterId;
        var list = cm.sql_Select(selectSql);
        if (list.size() == 0) {
            var mainIsertSql = "insert into equip_score_job(character_id,total_score,job,name) values (?, ?, ?, ?)";
            cm.sql_Update(mainIsertSql, cm.getPlayer().getId(), totalScore, cm.getPlayer().getJob(), cm.getPlayer().getName());
        } else {
            var hisScore = list[0].get("total_score");
            if (hisScore != totalScore) {
                //update
                var mainUpdateSql = "update equip_score_job set total_score=" + totalScore + " ";
                mainUpdateSql += " where character_id=" + characterId;
                cm.sql_Update(mainUpdateSql);
            }
        }
        cm.getPlayer().dropMessage(5, "您的战斗力信息已更新，您目前的战斗力为[" + totalScore+"]");
        cm.dispose();
    }
}