...
var prizeIdScroll = Array(2040502, 2040505,			// 卷轴
    2040802,										// Gloves for DEX
    2040002, 2040402, 2040602);						// Helmet, Topwear and Bottomwear for DEF
var prizeIdUse = Array(2000001, 2000002, 2000003, 2000006,	// 橙色，白色和蓝色药剂和魔法药剂
    2000004, 2022000, 2022003);						// Elixir, Pure Water and Unagi
var prizeQtyUse = Array(80, 80, 80, 50,
    5, 15, 15);
var prizeIdEquip = Array(1032004, 1032005, 1032009,			// Level 20-25 随机装备的地方
    1032006, 1032007, 1032010,						// Level 30 耳环
    1032002,										// Level 35 耳环
    1002026, 1002089, 1002090);						// 帽子
var prizeIdEtc = Array(4010000, 4010001, 4010002, 4010003,	// 矿石类
...
var xmitem = new Array(4002000,4002001,4002003,4001266,4021004); //"+cm.serverName()+"-新加入获取物品
var xmitemsl = new Array(1); //"+cm.serverName()+"-新加入获取物品数量
...
function action(mode, type, selection) {
			...
			if (preamble == null) {
				...
				if (complete != null) {
					...
					if (!cm.haveItem(4001008, numpasses)) {
						...
						clear(1,eim,cm);
						cm.givePartyExp(5000, party);//第一关
	...
	if (stage5done == null) {
		...
		if (passes) {
		    ...
		    clear(5,eim,cm);
		    cm.givePartyExp(10000, party);//最后一关经验
		    cm.dispose();
		} else { // Not done yet
...
function getPrize(eim,cm) {
    ...
    cm.gainItem(itemSet[sel], qty);//废弃组队任务领奖、获取物品的地方
	...
	gain积分("废弃积分",+1);
	gain积分("副本积分",+1);
	cm.getPlayer().setBossLog("废弃副本");//给一条龙次数
	...
	//cm.gainItem(xmitem[Math.floor(Math.random() * xmitem.length)],1); //"+cm.serverName()+"新加入随机获取物品	
