...
var PQLog = ["狂暴希拉"];//记录次數名称
var moblevel = 255;//怪物最大等级设置
var startmap = 450011990;//开始的地圖
...
function action(mode, type, selection) {
    ...
    if (cm.getMapId() == startmap) {
        if (status == 0) {
            ...
            rwpz += "#d       当前已挑战狂暴希拉[#e#g"+cm.getBossLog("狂暴希拉") + "#n#k/#r"+maxenter+"#n#k]次#n#k\r\n";
            rwpz += "\r\n#b推荐人數：" + minPlayers + " - " + maxPlayers + "#n    推荐等级：" + minLevel[selection] + " - " + maxLevel[selection] + "#n#k";
            rwpz += "\r\n#d当前已進行：#e#r" +cm.getBossLog("狂暴希拉")+ "#n#k次\r\n";
            ...
            if (cm.getParty() == null) { //判断组队
                ...
                cm.dispose();
                return;
            } else if ( cm.getBossLog("狂暴希拉") >= maxenter) {
                ...
                return;
                      ...
                      if (cm.getPlayerCount(450010100) <= 0 && cm.getPlayerCount(450010100) <= 0 && cm.getPlayerCount(450010100) <= 0 && cm.getPlayerCount(450010100) <= 0 && cm.getPlayerCount(450010100) <= 0) {
                            cm.warpParty(450010100);
                            cm.getMap(450010100).resetFully();
                            cm.spawnMob(8880400, 185, 266);
                            cm.getPlayer().setBossLog("狂暴希拉");
                            cm.喇叭(2,"『终极BOSS』：【" + cm.getChar().getName() + "】带领他的队伍悍不畏死的去狂暴希拉了");
                    } else {
