...
function action(mode, type, selection) {
	...
	if (status == 0) {
		...
		//text += icon1 + " 您的抵用券：#r[" + cm.getNX(2) + "]#d 点\r\n";
		...
		case 1:
			typed = 1;
			cm.sendYesNo("扩建花园需要#r1000#kW金币，扩建之后将会多出一个花盆，是否扩建？");
			break;
		case 2:
			typed = 2;
			...
			for (var key in flowers) {
				...
				if (flowers[key][1] > gardenData['level']) tips = "(需要" + flowers[key][1] + "级)#n#b";
		...
		if (typed == 0) {
			...
			if (seedid != 0 && seedid != -1) {
				...
				text += "#r#t" + seedid + "##k\r\n";
				text += tips + "\r\n";
				if (!flag) text += "#b#L0#" + icon2 + "浇水#l\t#L1#" + icon2 + "施肥#l\t#L2#" + icon2 + "挖除#l";
				else text += "#b#L4#" + icon2 + "采摘果实#l\t#L2#" + icon2 + "挖除#l";
				cm.sendOkS(text, 2);
			} else {
		...
		if (typed == 2) {
			...
			case 0:
				status -= 3;
				if (cm.getPlayer().getBossLog("花园浇水" + operationId) > 0) {
					cm.sendOkS("啊，好像浇过水了，不能拔苗助长！\r\n#L0##b" + icon2 + "点我返回#l", 2);
				} else {
					 if (cm.getPlayer().getBeans() > 1000) {
						cm.getPlayer().setBossLog("花园浇水" + operationId);
						cm.getPlayer().gainBeans(-1000);
						Operations(operationId, 0);
						cm.getPlayer().dropMessage(6, "消耗了1000个豆豆，缩短了1小时成长时间。");
						cm.sendOkS("好开心，花儿又长大了一点！\r\n#L0##b" + icon2 + "点我返回#l", 2);
					} else {
						cm.sendOkS("走开走开，你没有1000豆豆！\r\n#L0##b" + icon2 + "点我返回#l", 2);
					}
				}
				break;
				//施肥操作
			case 1:
				status -= 3;
				if (cm.getPlayer().getBossLog("花园施肥" + operationId) > 0) {
					cm.sendOkS("啊，好像施过肥了，不能拔苗助长！\r\n#L0##b" + icon2 + "点我返回#l", 2);
				} else {
					 if (cm.getPlayer().getBeans() >2000) {
						cm.getPlayer().setBossLog("花园施肥" + operationId);
						cm.getPlayer().gainBeans(-2000);
						Operations(operationId, 1);
						cm.getPlayer().dropMessage(6, "消耗了2000个豆豆，缩短了2小时成长时间。");
						cm.sendOkS("好开心，花儿又长大了一点！\r\n#L0##b" + icon2 + "点我返回#l", 2);
					} else {
						cm.sendOkS("走开走开，你没有2000豆豆！\r\n#L0##b" + icon2 + "点我返回#l", 2);
					}
				}
				break;
				//挖除操作
...
function clearFlowerPot(flowerpotid) {
	...
	cm.killbossloga("花园施肥" + flowerpotid);
	cm.killbossloga("花园浇水" + flowerpotid);
	var conn = cm.getConnection();
	var pstmt = conn.prepareStatement(sql);
