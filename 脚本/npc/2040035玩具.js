...
var itemList = new Array(1032055,1032056,1032057,1032058,1052165,1052166,1052167,1002797,1002800,1072366,1072368,1072262,1072264,1082244,1082245,1102174,1322003, //equips
                         ...
                         2043101,2043002,2043001,2040801,2040814,2040816,2040817,2040802,2040915,2040914,2040805,2040804,2040532,2040534,2040517,2040516,
...
function action(mode,type,selection) {
    ...
    if (mode == 1)
        ...
        status --;
    if (status == 0) {
        cm.sendNext("\t#d历经艰辛，方得始终，恭喜你，你就是最靓的");
    } else if (status == 1) {
	//cm.gainExp( + 500000);
	cm.gainExp(cm.getLevel()*6000);
	//cm.gainItem(4000487,1);//暗影币
	//cm.gainItem(4001465,2);
	cm.gainItem(4170005,1); 
	cm.setBossLog("MRWJ");
	Packages.handling.world.World.Broadcast.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(3, cm.getClient().getChannel(), "『组队任务 - 玩具副本』" + " : " + "[" + cm.getChar().getName() + "]成功通关玩具副本 获得超级丰富的奖励！！")); 
	cm.removeAll(4001022);
	cm.removeAll(4001023);
	cm.getPlayer().endPartyQuest(1202);//might be a bad implentation.. incase they dc or something
    cm.warp(221024500); //cm.gainItem(randItem, qty);  	cm.warp(221024500);
    cm.dispose();
    }
}
