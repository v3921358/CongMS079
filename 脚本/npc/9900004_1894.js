...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) {
			cm.sendNext("#r#e         "+小黄星+"   #e#d武 器 时 装 升 星#k#n   "+小黄星+"  #k#n\r\n  "+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+中条猫+"\r\n\r\n   #b欢迎来到#e" + cm.getChannelServer().getServerName() + "武器时装升星中心\r\n#k#n1.在我这里可以使用点卷加金币进行升星!\r\n2.需要升星的武器时装请放在装备栏第一个格!#k\r\n");
				} else if (status == 1) {
			...
			if(cm.getInventory(1).getItem(1)== null ){
             ...
             cm.sendOk("第一格装备是#v" + item.getItemId() + "#,不是能升星的武器属性时装!");
			 cm.dispose();    
			 return;
            }
				...
				sj = Math.floor(Math.random()*40);//随机100%
				shibai = 0;
				xxnew = "★";
				点卷数量 = 800000;
				金币数量 = 100000;
				...
				sj = Math.floor(Math.random()*35);//随机80%
				shibai = 0;
				xxnew = "★★";
				xxdj = 1;
				点卷数量 = 1600000;
				...
				sj = Math.floor(Math.random()*32);//随机60%
				shibai = 0;
				xxnew = "★★★";
				xxdj = 2;
				点卷数量 = 2000000;
				...
				sj = Math.floor(Math.random()*30);//随机40%
				shibai = 0;
				xxnew = "★★★★";
				xxdj = 3;
				点卷数量 = 3000000;
				...
				sj = Math.floor(Math.random()*25);//随机20%
				shibai = 0;
				xxnew = "★★★★★";
				xxdj = 4;
				点卷数量 = 4000000;
				...
				sj = Math.floor(Math.random()*25);//100%
				shibai = 0;
				xxnew = "★★★★★★";
				xxdj = 5;
				点卷数量 = 8000000;
