...
function action(mode, type, selection) {
	...
	if (selection == 0) {
		...
		} else {     //for循环，清除全队背包副本字母物品
            for (var i = 3994059; i < 3994085; i++) {
	        cm.givePartyItems(i, 0, true);
	        }
			if(cm.getPlayer().getMapId()==702090101){
		    cm.givePartyItems(4001137, 1);
		    cm.warpParty(702090400, 0);
            cm.showEffect(true, "englishSchool/correct");
			}
			if(cm.getPlayer().getMapId()==702090201){
		    cm.givePartyItems(4001137, 2);
		    cm.warpParty(702090400, 0);
            cm.showEffect(true, "englishSchool/correct");
			}
			if(cm.getPlayer().getMapId()==702090301){
		    cm.givePartyItems(4001137, 3);
		    cm.warpParty(702090400, 0);
            cm.showEffect(true, "englishSchool/correct");
			}
		}
