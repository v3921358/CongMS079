...
function action(mode, type, selection) {
	if (mode == -1) {
        ...
        cm.sendSimple(text);
			...
			MapleInventoryManipulator.addFromDrop(cm.getC(), item,false);
			cm.sendOk("星之力强化成功！");
			Packages.handling.world.World.Broadcast.broadcastMessage(Packages.tools.MaplePacketCreator.serverNotice(9,cm.getC().getChannel(),"[进阶星之力]" + " : " + " 玩家 [" + cm.getPlayer().getName() + "]成功强化星之力至"+xxnew+"！大家祝贺他(她)吧！",true).getBytes()); //喇叭  
			cm.dispose();
		}
	}
}
