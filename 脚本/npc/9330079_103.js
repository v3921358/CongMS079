...
function action(mode, type, selection) {
		...
		if (status == 0) {
			var text = "我是孔明，答题时间为周三#r20:30-20:40#n#k。\r\n答题上限100题请认真回答每一题~#b\r\n";
			...
			if(selection==3){// 38道 1国庆币    58道 1暗影币      68道 1祝福卷     78道 1混沌   88道勋章
				...
				selText += "#L1# 换取 点卷:10000     (需答对 30道)#l\r\n\r\n";
				selText += "#L2# 换取 点卷:20000     (需答对 50道)#l\r\n\r\n";
			...
			if(typed == 1){
				...
				if(selection == tknow[5] || fee == tknow[5]){
					...
					var randy = Math.floor(Math.random() * 10000) + 10000;//冒险币
					var randdj = Math.floor(Math.random() * 100)+100;//点卷
					var randdy = Math.floor(Math.random() * 10) + 10;//点卷
					...
					if(randx >= 12 && randx<= 15){//枫叶
						...
						cm.sendOk("恭喜，您答对了！#b正确答案:"+tknow[5]+"#k\r\n您获得了 " +randred+" 枫叶.");
					        cm.喇叭(1, "[每日答题]：恭喜：" + cm.getPlayer().getName() + "正确回答<诸葛孔明>的问题,得到 " +randred+"枫叶~");
					}					
				}else{
					cm.sendOk("很遗憾，回答错误！\r\n");
			...
			if(typed==2){
				if(selection==1){//// 38道 1国庆币4000463   
					if (cm.getPlayer().getBossLog("答题38兑换") < 1){
						if (cm.getPlayer().getBossLog("答题正确") < 30){
						...
						cm.喇叭(1,"[每日答题]:恭喜["+ cm.getChar().getName() +"]用正确答题数目30题兑换了1万点卷");
					} else {
						...
						if (cm.getPlayer().getBossLog("答题正确") < 50){
							cm.sendOk("抱歉，您今天还没有答对50道题目，请继续努力！\r\n");
							cm.dispose();
							return;
						}
						cm.gainNX(20000);
						...
						cm.sendOk("恭喜，您使用正确答题数目兑换成功!");
						cm.喇叭(1,"[每日答题]:恭喜["+ cm.getChar().getName() +"]用正确答题数目50题兑换了2万点卷~");
					} else {
						cm.sendOk("抱歉，您今天已经领取过了.");
						cm.dispose();
						return;
						...
						cm.gainItem(4032392, 1);//破碎2
						
						cm.getPlayer().setBossLog("答题78兑换");
						cm.sendOk("恭喜，您使用正确答题数目兑换成功!");
						cm.喇叭(1,"[每日答题]:恭喜["+ cm.getChar().getName() +"]用正确答题数目78题兑换了 破碎的卷轴碎片2~");
					} else {
						cm.sendOk("抱歉，您今天已经领取过了.");
						cm.dispose();
						return;
