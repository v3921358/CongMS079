...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        if (selection == 3) {
            ...
            var sql = "select g.guildid, g.name, l.rvalue from guilds g, ranklist l where g.guildid = l.rid and l.rtype = 1 and WEEKOFYEAR(rtime) = WEEKOFYEAR(NOW()) - ? order by l.rvalue desc limit 9";
            var list = cm.sql_Select(sql, selection);
            var text = "\t\t\t\t#e#d"+eff+" - 家族排行 - "+eff+"#k#n\r\n\r\n";
            var i = 1;
            for (var ii in list) {
                ...
                text += "\t#b " + list[ii].get("rvalue")+" GP";
                for (var j = 16 - (list[ii].get("rvalue")+" GP").toString().getBytes().length; j > 0; j--) {
                    text += " ";
                }
                text += "\t #k";
                text += "\r\n";
        ...
        if (selection == 99) {
            if (cm.getPlayer().getGuild().getLeaderId() != cm.getPlayer().getId()) {
                ...
                cm.sendOk("恭喜你领取成功");
                Packages.server.custom.bossrank.BossRankManager.getInstance().addRanklistGP(cm.getPlayer().getGuildId(), 3, 1);
            }
        }
        cm.dispose();
    }
...
function getlog(id) {
    var sql = cm.sql_Select("select * from ranklist where rid = ? and rtype = ? and WEEKOFYEAR(rtime) = WEEKOFYEAR(NOW())", id, 3);
    if (sql.length > 0) {
        return 1;
    } else {
        return 0;
