...
var PQLog = ["桃乐丝"];//记录次數名称
var moblevel = 255;//怪物最大等级设置
var startmap = 992000000;//开始的地圖
...
function action(mode, type, selection) {
    ...
    if (cm.getMapId() == startmap) {
        if (status == 0) {
            ...
            rwpz += "#d       当前已挑战桃乐丝[#e#g"+cm.getBossLog("桃乐丝") + "#n#k/#r#e3#n#k]次#n#k\r\n";
            rwpz += "\r\n#b推荐人數：" + minPlayers + " - " + maxPlayers + "#n    推荐等级：" + minLevel[selection] + " - " + maxLevel[selection] + "#n#k";
            rwpz += "\r\n#d当前已進行：#e#r" +cm.getBossLog("桃乐丝")  + " #n#k次\r\n";
            ...
            if (cm.getParty() == null) { //判断组队
                ...
                cm.dispose();
                return;
            } else if (cm.getLevel() < 160) {//判断玩家角色等级
                cm.sendNext("组队成员等级 " + 160 + " 以上 " + 255 + " 以下才可以入场。");
                cm.dispose();
                return;
            } else if ( cm.getBossLog("桃乐丝") >= 3) {
                ...
                return;
                      ...
                      if (cm.getPlayerCount(992050000) <= 0 && cm.getPlayerCount(992050000) <= 0 && cm.getPlayerCount(992050000) <= 0 && cm.getPlayerCount(992050000) <= 0 && cm.getPlayerCount(992050000) <= 0) {
                            cm.warpParty(992050000);
                            cm.getMap(992050000).resetFully();
                            cm.spawnMob(9309207, 543, 185);
                            cm.getPlayer().setBossLog("桃乐丝");
                            cm.喇叭(2,"『终极BOSS』：【" + cm.getChar().getName() + "】带领他的队伍悍不畏死的去桃乐丝了");
                    } else {
