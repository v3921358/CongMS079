...
var successRate = 100;
//费用
var mose =5000000,mose2=5000000;//金币 
var cost = 1000;//点卷
...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
			...
			//显示计算后的合成成功率以及所需要的费用
			text += "\r\n#L990##b"+startIcon+" 继承成功率：#r"+Surate+"#b% 所需费用：#r500万#b金币#l\r\n";
			text += "\r\n#L991##b"+startIcon+" 继承成功率：#r"+Surate1+"#b% 所需费用：#r500万#b金币 #r1000#b点卷#l\r\n";
			//显示确定按钮
			...
			if (selection >= 990) {
				...
				if (selectedList.length < numberno||selectedList[1]==null) {
					...
					if (ItemPosition==999 && !haveLuck) {
						...
						//Math.random()*100取0-100中随机数   Math.floor取一个整数，1.2  就取1
						...
						if (chance > successRate) {
							...
							for(var key in selectedList) {
								if (key==0)
									continue;
								var breakRate = Math.floor(Math.random()*99);//随机0-99
								if (breakRate <= 49) {
									indexof++;
							...
							if (indexof > 0)  //不带{}表示 后面只有一句属于if语句范围内的语句
				...
				else if(selectedPosition==2)
				{
					...
					while (itemList.hasNext()) {
						...
						if(item.getExpiration() != -1)continue;//加这个  就限时的不显示
