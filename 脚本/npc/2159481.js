...
function action(mode, type, selection) {
    ...
    if (cm.getMapId() == startmap) {
        if (status == 0) {
            ...
            rwpz += "#d       当前已挑战路西德[#e#g"+cm.getBossLog("路西德") + "#n#k/#r#e"+maxenter+"#n#k]次#n#k\r\n";
            rwpz += "\r\n#b推荐人數：" + minPlayers + " - " + maxPlayers + "#n    推荐等级：" + minLevel[selection] + " - " + maxLevel[selection] + "#n#k";
            rwpz += "\r\n#d当前已進行：#e#r" +cm.getBossLog("路西德")  + " #n#k次\r\n";
            ...
            if (cm.getParty() == null) { //判断组队
                ...
                cm.dispose();
                return;
            } else if ( cm.getBossLog("路西德") >= maxenter) {
                ...
                return;
                      ...
                      //if (cm.getPlayerCount(882100002) <= 0 && cm.getPlayerCount(882100003) <= 0 && cm.getPlayerCount(882100004) <= 0 && cm.getPlayerCount(882100004) <= 0 && cm.getPlayerCount(882100004) <= 0) {
						if (cm.getPlayerCount(882100002) <= 0 ) {
                            cm.warpParty(882100002);
                            cm.getMap(882100002).resetFully();
                            cm.spawnMob(8880140, 750, 80);
                            cm.getPlayer().setBossLog("路西德");
                            cm.喇叭(2,"『终极BOSS』：【" + cm.getChar().getName() + "】带领他的队伍悍不畏死的去路西德了");
                    } else {
