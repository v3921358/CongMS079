...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (status == 0) {
			score = cm.getPlayer();
			var text = "";
			text += "\t\t\t" + hx + hx + hx + "#e#r豆豆开心中心#n#k" + hx + hx + hx + "\r\n ";
			text += "#fUI/UIWindow/Quest/icon3/6# ";
			text += "我这里可以玩21点,";
			text += "您当前剩余豆豆为 #r" +cm.getBeans()+ "个#k";
			text += " #fUI/UIWindow/Quest/icon3/6#\r\n";
			text += "#L0# 开始游戏 #l\r\n";
			text += "#L1# 游戏说明 #l\r\n";
			cm.sendSimple(text);
			...
			if(selection == 0){		
				...
				text1 += "玩法说明：";
				text1 += " #fUI/UIWindow/Quest/icon3/6#\r\n";
				text1 += "    庄家的点数会在15点-22点之间（庄家也有可能爆掉），你有4次要牌的机会，如果您的点数大于庄家的点数即赢得你下注的金额，如果大于21点或者小于等于庄家的点数，即输掉你所下注的金额，10、J、Q、K都算十点，开心娱乐，请慎重不要沉迷。";
				cm.sendOk(text1);
				cm.dispose();
			}
		}
		//-------------第一次要牌-----------------//
			...
			selectionBeans = selection;
			if(selectionBeans > cm.getBeans() ){
				cm.sendOk("你想来坑我么，没豆了！保安，给我拖出去！");
				cm.dispose();
			}else{
				...
				cm.sendSimple(text2);
				cm.gainBeans(-selectionBeans);
				cm.playerMessage(5, "已经扣除投入："+selectionBeans+" ");
			}
		...
		//------------第二次要牌------------------//
			...
			if(selection == 2){
				...
				if((xianScore<22 && xianScore<=zhuangScore && zhuangScore<22) || (xianScore>=22 && zhuangScore>=22)){
					...
					//cm.worldMessage(6,"【豆豆屋-绿蘑菇机】["+cm.getName()+"] 在赌场惨输" + selectionBeans + "个豆豆，运气真是差呀!!"); //
					
					cm.dispose();
				}else{
					cm.sendOk("#fUI/UIWindow/Quest/reward#\r\n  庄家的点数为 #r#e"+zhuangScore+"#n#k,您的点数为 #r#e"+xianScore+"#n#k,您赢了。");
					cm.gainBeans(selectionBeans*2);
					cm.worldMessage(6,"【豆豆屋-绿蘑菇机】["+cm.getName()+"] 在豆豆开心中心赢了" + selectionBeans + "个豆豆,人品大爆发了!一起恭喜他吧!!"); //
					cm.dispose();	
				}
			}
